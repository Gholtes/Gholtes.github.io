...
function wordleSearch(knownLetters, includedLetters, notIncludedLetters) {
	var i;
	var j;
	var matches = []
	var match_included;
    ...
    for (i=0; i < matchingWords.length; i++) {
		...
		match_included = true;
		//Checl known letters and location
        if (matchingWord.match(regex)) {
			//Check known letters regardless of location
			for (j=0; j < includedLetters.length; j++){
				includedLetter = includedLetters[j];
				if (matchingWord.indexOf(includedLetter) === -1) {
...
function ingestUserInput() {
	...
	var known3 = document.getElementById("known3").value;
	var known4 = document.getElementById("known4").value;
	var known5 = document.getElementById("known5").value;

	var lettersInclude = document.getElementById("LettersInclude").value;
